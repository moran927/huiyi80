...
.box-play i.ban-close { width: 30px; height: 30px; top:10px; right:0; background: url(/static/zhe800/images/good/ban-esc.png) no-repeat; }
.box-play i.ban-close:hover { background: url(/static/zhe800/images/good/ban-esc.png) no-repeat 0 -30px; }
.box-play i.ban-open { width:25px; height:18px; right:-25px; top:3px; background: url(/static/zhe800/images/good/open.gif) no-repeat; }
.box-play .count-num { position:absolute; bottom:2px; right:14%; }      
.box-play .count-num li { float:left; _display:inline; margin-left:10px; cursor:default; background: url(/static/zhe800/images/good/switch2.gif) no-repeat -16px 0;width: 15px; height: 15px; border-radius: 10px; background: #fff; opacity: .8; _filter: Alpha(opacity = 80); overflow:hidden; }
