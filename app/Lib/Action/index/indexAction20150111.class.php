...
class indexAction extends FirstendAction {
    ...
    public function index() {
		$article=M('article')->order('add_time desc')->limit(10)->select();//文章
  ...
  /**
	...
	public function cate(){
		...
		if($scid)
		{
		    $scinfo = $this->_cate_mod->where(array('id'=>$scid))->find();
			$tag.="&nbsq;".$scinfo['name'];
		}
		//风格id
		if($fgid)
		{
		    $fginfo = $this->_cate_mod->where(array('id'=>$fgid))->find();
			$tag.="&nbsq;".$fginfo['name'];
		}
		//流行id
		if($lxid)
		{
		    $lxinfo = $this->_cate_mod->where(array('id'=>$lxid))->find();
			$tag.="&nbsq;".$lxinfo['name'];
		}
		...
		if($mother !='0')
		{
		   $marray=explode('|',$mother);
		   $motherid=$marray[0];
		   $moth=$this->_cate_mod->where(array('id'=>$motherid))->find();
		   $mothname=$moth['name'];
		...
		$this->assign('motherid',$motherid);
		$this->assign('cid',$cid);
		if($mothname)
		{
		    
		
		   $tagss="<a href='".U('index/cate',array('cid'=>$motherid,'scid'=>$scid,'fgid'=>$fgid,'lxid'=>$lxid))."'>".$moth['name']."</a> <i class='last'>&gt;</i>  
		   ".$cinfo['name']."&nbsp;".$tag;
		}
		else
		{
		...
		$this->assign('navname',$tagss);
        $catlist=D("items_cate")->get_child_idswqc($motherid,false);
		$this->assign('catlistchild',$catlist);
