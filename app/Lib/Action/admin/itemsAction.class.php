...
class itemsAction extends BackendAction {
    ...
    public function edit() {
        if (IS_POST) {
            ...
            $spidfg = $this->_cate_mod->where(array('id'=>$item['cate_idfg']))->getField('spid');
            if( $spidfg==0 ){
                $spidfg = $item['cate_idfg'];
            }else{
                $spidfg .= $item['cate_idfg'];
            }
            $this->assign('selected_idsfg',$spidfg); //分类选中
            $spidsc = $this->_cate_mod->where(array('id'=>$item['cate_idsc']))->getField('spid');
            if( $spidsc==0 ){
                $spidsc = $item['cate_idsc'];
            }else{
                $spidsc .= $item['cate_idsc'];
            }
            $this->assign('selected_idssc',$spidsc); //分类选中
             $spidlx = $this->_cate_mod->where(array('id'=>$item['cate_idlx']))->getField('spid');
            if( $spidsc==0 ){
                $spidlx = $item['cate_idlx'];
            }else{
                $spidlx .= $item['cate_idlx'];
            }
            $this->assign('selected_idslx',$spidlx); //分类选中
