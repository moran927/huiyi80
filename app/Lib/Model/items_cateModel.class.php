...
class items_cateModel extends Model
{
    ...
    public function get_child_idswqc($id, $with_self=false) {
		...
		//}else{
			...
			if(F('get_child_idswqc_'.$id))
		   {
			    return F('get_child_idswqc_'.$id);
			}
			else
		   {
			$id_arr = $this->field('id,spid,name,pid')->where(array('pid'=>$id))->select();
			$array = array();
			$zm=array(0 =>'a',1=>'b',2=>'c',3=>'d');
		    ...
		    {   
			    ...
			    $spids = $this->field('id,spid,name,pid')->where(array('pid'=>$value['id']))->select();
				foreach($spids as $key => $values)
				{
                     $array[$values['pid']]['child'][]=$values;
				}
     ...
     * 分类关系读取写入缓存wqcfyl
     */
    public function relate_cachewqc() {
		if(F('cate_relate_wqc'))
		{

			return F('cate_relate_wqc');
		}
		else
		{
        $cate_relate = array();
        $cate_data = $this->field('id,pid,spid,name')->where('status=1 and pid=0')->order('ordid')->select();
        foreach ($cate_data as $val) {
			$cate_relate[$val['id']]['name']=$val['name'];
			$cate_relate[$val['id']]['id']=$val['id'];
            $cate_relate[$val['id']]['child']= $this->get_child_idswqc($val['id']); //子孙 几个流行元素
          
        }

        F('cate_relate_wqc', $cate_relate);
        return $cate_relate;
		}
    }
    /**
