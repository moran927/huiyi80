...
<div class="jiu-nav-main">
	<div class="jiu-nav {:C('ftx_site_width')}">	<
		<div class="nav-item fl">
			<div class="item-list">
				<ul>
				<li><a href="{:U('yugao/index')}" <if condition="$nav_curr eq yugao">class="active"</if>>全部</a></li>
				<?php  $cdi=0; ?>
		...
		<div class="n_h">
			<span>排序：</span>
			<a href="{:U('yugao/index')}"  class=" <if condition="$index_info['sort'] eq 'default'"> active </if> ">默认</a>
			<a href="{:U('yugao/index',array('sort'=>'new'))}" class=" <if condition="$index_info['sort'] eq 'new'"> active </if> ">最新</a>
			<a href="{:U('yugao/index',array('sort'=>'hot'))}" class=" <if condition="$index_info['sort'] eq 'hot'"> active </if> ">最热</a></div>
		</div>
