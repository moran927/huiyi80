    ...
    <div class="tag_nav_shopping col-{$bcates.zm}">
	    ...
	    <a href="{:U('index/cate',array('cid'=>$motherid))}" class="cur">所有</a>
            ...
            <if condition="$bcates['name'] eq '潮流单品'">
             ...
             <if condition="$bcatew['cid'] eq $cid">
	     <li><a href="{:U('index/cate',array('cid'=>$bcatew['id'],'scid'=>$scid,'fgid'=>$fgid,'lxid'=>$lxid))}" class="cur"><strong>{$bcatew.name}</strong></a></li>
	     <else />
	     <li><a href="{:U('index/cate',array('cid'=>$bcatew['id'],'scid'=>$scid,'fgid'=>$fgid,'lxid'=>$lxid))}"><strong>{$bcatew.name}</strong></a></li>
	     </if>
              </volist>
             </if>

	     <if condition="($bcates['name'] eq '材质') or ($bcates['name'] eq '身材') or  ($bcates['name'] eq '跟型')">
             <volist name="bcates.child" id="bcatew">
             <if condition="$bcatew['scid'] eq $scid">
	     <li>1<a href="{:U('index/cate',array('cid'=>$bcatew['id'],'scid'=>1,'fgid'=>2,'lxid'=>3))}" class="cur"><strong>{$bcatew.name}</strong></a></li>
	     <else />
	     <li>2<a href="{:U('index/cate',array('cid'=>$bcatew['id'],'scid'=>3,'fgid'=>4,'lxid'=>5))}"><strong>{$bcatew.name}</strong></a></li>
	     </if>
              </volist>
             </if>

	      <if condition="$bcates['name'] eq '风格'">
             <volist name="bcates.child" id="bcatew">
             <if condition="$bcatew['fgid'] eq $fgcid">
	     <li><a href="{:U('index/cate',array('cid'=>$bcatew['id'],'scid'=>$scid,'fgid'=>$fgid,'lxid'=>$lxid))}" class="cur"><strong>{$bcatew.name}</strong></a></li>
	     <else />
	     <li><a href="{:U('index/cate',array('cid'=>$bcatew['id'],'scid'=>$scid,'fgid'=>$fgid,'lxid'=>$lxid))}"><strong>{$bcatew.name}</strong></a></li>
	     </if>
	      ...
	      <if condition="$bcates['name'] eq '流行'">
             <volist name="bcates.child" id="bcatew">
             <if condition="$bcatew['lxid'] eq $lxid">
	     <li><a href="{:U('index/cate',array('cid'=>$bcatew['id'],'scid'=>$scid,'fgid'=>$fgid,'lxid'=>$lxid))}" class="cur"><strong>{$bcatew.name}</strong></a></li>
	     <else />
	     <li><a href="{:U('index/cate',array('cid'=>$bcatew['id'],'scid'=>$scid,'fgid'=>$fgid,'fgid'=>$lxid))}"><strong>{$bcatew.name}</strong></a></li>
	     </if>
              </volist>
             </if>
